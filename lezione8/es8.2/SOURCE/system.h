...
private:
  ...
  int _SAblocks;        // Number di blocchi simulazione SA
  ...
  double _acceptance;   //probabilità di accettazione metodo SA
...
public: // dichiarazione delle funzioni
  ...
  double getH(); //restituisce la media cumulativa finale di una simulazione MC di <H>
  double get_error_H(); //restituisce l'errore sulla stima finale di una simulazione MC di <H>
  double getSAblocks(); //restituisce il numero di blocchi dela simulazione SA
  bool SAmove(double H1, double H2); //effettua una mossa di simulated annealing
  double get_acceptance(); //restituisce la percentuale di mosse accettate
  double Rannyu(double min, double max); //chiamo il metodo Rannyu della classe Random
