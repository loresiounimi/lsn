...
private:
  ...
  int _SAblocks;        // Number of blocks for SA
...
public: // Function declarations
  ...
  double PSI(int i, bool xnew); 
  double PSI_seconda(int i, bool xnew); 
  void set_temperature(double t); // set temperature
  double get_temperature();  //get temperature
  void set_media(double media);
  double get_media();
  void set_sigma(double sigma);
  double get_sigma();
  void set_beta(double t);
  void global_reset();
  double getH();
  double get_error_H();
  double getSAblocks();
  bool SAmove(double H1, double H2);
