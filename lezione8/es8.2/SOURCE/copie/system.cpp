...
bool System :: metro(int i){ // Metropolis algorithm
  bool decision = false;
  double acceptance;
  if(_sim_type == 1) acceptance = pow(fabs(this->PSI(i,true)),2)/pow(fabs(this->PSI(i,false)),2);
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    if(coutf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        coutf << "LJ" << "  " 
              << setprecision(17) << _particle(i).getposition(0,true)/_side(0) << endl; //"   " // x
    ...
    if(coutf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        coutf << "LJ" << "  "
              << setprecision(17) << _particle(i).getposition(0,false)/_side(0) << endl; //"   " // x
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      cinf >> particle >> x; // units of coordinates in conf.xyz is _side
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      for (int i=0; i<_npart; i++){
        ...
        tenergy_temp += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j),2;
      }
  ...
  if (_measure_H){
    if (_sim_type != 1)  { 
      cout << "simulation type errato!" << endl;
       }
