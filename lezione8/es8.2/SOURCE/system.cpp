...
//per il calcolo di <H> e dei migliori parametri mu e sigma per minimizzare <H>
#include <cmath>
#include <cstdlib>
#include <string>
#include "system.h"
...
void System :: initialize(){ // inizializza il sistema in base al contenuto dei file di input che si trovano nella cartella INPUT
  ...
  while ( !input.eof() ){
    ...
    if( property == "RESTART" ){ //restart dice al sistema se generare valori per le velocità e dunque per le posizioni precdenti oppure se leggere le posizioni precedenti da un file già esistente
      ...
      input >> _temp;
      _beta = 1.0/_temp;
      coutf << "TEMPERATURA_iniziale= " << _temp << endl;
      ...
      input >> _media;
      coutf << "MEDIA_iniziale= " << _media << endl;
    } else if( property == "SIGMA" ){ //valore di sigma
      input >> _sigma;
      coutf << "SIGMA_iniziale= " << _sigma << endl;
    } else if( property == "SABLOCKS" ){ //numero di blocchi per simulazione SA
      input >> _SAblocks;
      coutf << "SABLOCKS= " << _SAblocks << endl;
...
//calcolo delle medie e dell'incertezza
...
//restituisce la media cumulativa finale di una simulazione MC di <H>
double System ::  getH(){
  return _global_av(_index_H)/_nblocks;
}
//restituisce l'errore sulla stima finale di una simulazione MC di <H>
double System :: get_error_H(){
  return this->error(_global_av(_index_H), _global_av2(_index_H), _nblocks);
}
//restituisce il numero di blocchi della simulazione SA
double System ::  getSAblocks(){
  return _SAblocks;
}
//mossa di simulated annealing
bool System :: SAmove(double H1, double H2){
  bool decision = false;
  double acceptance;
  //calcolo della probabilità di accettazione
  acceptance = exp(-_beta * (H2 - H1));
  if(_rnd.Rannyu() < acceptance ) decision = true; //step di accettazione
  return decision;
}
//restituisce la percentuale di mosse accettate
double System :: get_acceptance(){
  return _acceptance;
 } 
//chiama il metodo Rannyu(min,max) del generatore di numeri casuali
double System :: Rannyu(double min, double max){
