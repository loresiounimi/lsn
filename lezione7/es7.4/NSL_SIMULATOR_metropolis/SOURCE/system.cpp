...
//in questo esercizio è effettuatta una simulazione una simulazione MC con potenziale LJ
#include <cmath>
#include <cstdlib>
#include <string>
#include "system.h"
...
void System :: move(int i){ // propone una mossa MC alla particella i
  ...
  } else { //algoritmo di metropolis per simulazione Montecarlo
    if(_sim_type == 1){    
      //propone la mossa MC   
      vec shift(_ndim);       
...
bool System :: metro(int i){ // algoritmo di metropolis
  ...
  //differenza energetica tra posizione iniziale e posizione proposta con la mossa MC
...
//calcolo l'energia potenziale di interazione tra la particella i e tutte le altre particelle del sistema,
//calcolata tramite il potenziale di Lennard-Jones, tenendo conto delle condizioni periodiche e usando un cut-off 
...
void System :: initialize(){ // inizializza il sistema in base al contenuto dei file di input che si trovano nella cartella INPUT
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){ //inizializza il sistema in base al tipo di simulazione
      ...
      else if(_sim_type == 1) coutf << "LJ MONTE CARLO (NVT) SIMULATION"         << endl; //simulazione MC con potenziale Lennard-Jones
  ...
  input.close();
  this->read_configuration();//leggo configurazione iniziale
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){//energia potenziale
        ...
        index_property++;
        _vtail = ((8.0*M_PI*_rho)/9.0)*((1.0/pow(_r_cut,9.0))-3.0*(1.0/pow(_r_cut,3.0)));
        ...
        index_property++;
        _ptail = 32.0*M_PI*_rho*(1.0/pow(_r_cut,9.0)-1.0/(6.0*pow(_r_cut,3.0)));
      } else if( property == "GOFR" ){//distribuzione radiale
...
if (_measure_penergy or _measure_pressure or _measure_gofr) {//se una di queste 3 proprietà è stata letta dal file properties allora la misuro
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        //misura di g(r)
        if(_measure_gofr){
          //aggiorno i conteggi nei bins
          for(int h=0;h<_n_bins;h++){
            if(dr>=(h*_bin_size)&&dr<((h+1)*_bin_size)){
            _measurement(_index_gofr+(h))+=2;
          }
          }
          }
        if(dr < _r_cut){
  ...
  if(_measure_gofr){
    for(int j=0;j<_n_bins;j++){
      //((4.0/3.0)*M_PI*(pow(((j+0.5)*_bin_size+_bin_size),3)-pow(((j+0.5)*_bin_size),3)) è il volume dell'anello sferico. Entra nella normalizzazione perchè Anche se le particelle sono uniformemente distribuite, 
      //ci saranno più particelle a distanza grande solo perché il volume del guscio aumenta con r
            _measurement(_index_gofr+(j))/=(_rho*(_npart)*((4.0/3.0)*M_PI*(pow(((j+0.5)*_bin_size+_bin_size),3)-pow(((j+0.5)*_bin_size),3))));
          }
     }
  ...
  if (_measure_kenergy){
     if (_sim_type == 1) {
      // in Monte Carlo NVT si usa la temperatura 
      kenergy_temp = (3.0/2.0) * _temp;
      _measurement(_index_kenergy) = kenergy_temp;
    } else {
      //in MD si usano le velocità
  ...
  // energia totale 
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;//formula per simulazione MD o MC
...
void System :: averages(int blk){
  ...
  if (_measure_gofr){
    ...
    string fname_blk = "../OUTPUT/gofr/gofr_block_" + to_string(blk) + ".dat";
    ...
    coutf << setw(12) << (_bin_size/2)+i*_bin_size
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
        }
    coutf.close();
    //salvo g(r) in un unico file
