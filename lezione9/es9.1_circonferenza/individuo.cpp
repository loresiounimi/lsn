...
mat Individuo :: genera_individuo_iniziale(){
    ...
    //generazione primo individuo con le città poste su una circonferenza unitaria
    for(int i=0;i<_cities;i++){
        theta=_rnd->Rannyu(0.0,2.0*M_PI);
        _individuo(i,0)=i+1;
        _individuo(i,1)=cos(theta);
        _individuo(i,2)=sin(theta);
        }
