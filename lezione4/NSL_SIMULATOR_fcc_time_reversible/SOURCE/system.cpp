...
#include <string>
#include "system.h"
//questa libreria mi serve per copiare i dati della simulazione forward time dalla directory OUTPUT a FORWARD_TIME
//in questo modo potrò memorizzare i dati delle simulazioni forward e bacward e creare grafici
#include <filesystem>
...
void System :: initialize(){ // inizializza il sistema in base al contenuto dei file di input che si trovano nella cartella INPUT
  ...
  input.close();
  if(_restart==1){//leggo dati per simulazione backward
    this->read_configuration_timereversible();
    }else{//leggo dati per simulazione forward
    this->read_configuration();
    }
  if(_sim_type==0) this->initialize_velocities();//inizializzo le velocità
...
void System :: finalize(){//finalizzo il sistema. In questo esercizio finalizza la simulazione bacward
  ...
  coutf << "Simulation backward completed!" << endl;
  coutf.close();
  return;
}

// scrivo la posizione finale delle particelle in un file.xyz nella cartella ../OUTPUT/CONFIG/. In questo esercizio agisce per la simulazione backward
...
// leggo le posizioni iniziali da un file in ../OUTPUT/CONFIG/. In questo esercizio usata per la simulazione backward
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    cinf >> comment;
    for(int i=0; i<_npart; i++){//carico le posizioni iniziali in modo che le particelle occupino solo metà del box della simulazione. Stato a bassa entropia
      cinf >> particle >> x >> y >> z; 
...
//aggiunte per esercizio 4.3 
//questa funzione finalizza il sistema dopo la simulazione in avanti nel tempo in modo da preparare i file per la simulazione indietro nel tempo
void System :: finalize_timereversible(){
  this->write_configuration_timereversible();//scrive le configurazioni finali su file
  _rnd.SaveSeed();
  ofstream coutf;
  coutf.open("../OUTPUT/output.dat",ios::app);
  coutf << "forward in time simulation completed!" << endl;
  coutf.close();
  _restart=true;
  //funzione della libreria filesystem che copia i dati da un file a un altro. In questo modo i dati della simulazione forward vengono memorizzati e non sovrascritti
  fs::copy_file("../OUTPUT/temperature.dat", "../OUTPUT/FORWARD_TIME/temperature.dat", fs::copy_options::overwrite_existing);
  fs::copy_file("../OUTPUT/kinetic_energy.dat", "../OUTPUT/FORWARD_TIME/kinetic_energy.dat", fs::copy_options::overwrite_existing);
  fs::copy_file("../OUTPUT/potential_energy.dat", "../OUTPUT/FORWARD_TIME/potential_energy.dat", fs::copy_options::overwrite_existing);
            
  return;
}

//questa funzione scrive le configurazioni finali della simulazione in avanti nel tempo in modo che xold della simulazione in avanti nel tempo venga letto come x_i della simulazione indietro nel tempo e
//x_f della simulazione in avanti nel tempo vengano lette come xold nella simulazione indietro nel tempo. è la funzione cruciale per l'inversione della linea temporale della simulazione
void System :: write_configuration_timereversible(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open("../INPUT/CONFIG/conf-1.xyz"); //per invertire il tempo salvo le posizioni attuali in un file di posizioni vecchie
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    coutf.open("../INPUT/CONFIG/config_timereversible.xyz"); //per invertire il tempo salvo le posizioni vecchie in un file di posizioni attuali
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    } else cerr << "PROBLEM: Unable to open config_timereversible.xyz" << endl;
    coutf.close();
  } else {//salva gli spin delle particelle, in questo caso non invertiti perchè non usati nell'esercizio
...
//questa funzione legge le configurazioni iniziali della simulazione indietro nel tempo
void System :: read_configuration_timereversible(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config_timereversible.xyz");//sono le xold della simulazione in avanti nel tempo
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & config_timereversible.xyz not match!" << endl;
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
  ...
  } else cerr << "PROBLEM: Unable to open INPUT file config_timereversible.xyz"<< endl;
  cinf.close();
