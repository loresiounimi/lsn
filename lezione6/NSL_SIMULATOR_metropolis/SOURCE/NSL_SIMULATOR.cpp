...
//simulazione di un modello di Ising 1D con algoritmo di Metropolis
#include <iostream>
#include "system.h"
...
int main (int argc, char *argv[]){
  ...
  for(int l=0;l<26;l++){//ciclo sulle temperature
    ...
    SYS.block_reset(0);//inizializzo gli accumulatori di blocco del blocco 0 a zero all'inizio di ogni simulazione per ogni T
  ...
  for(int i=0; i < SYS.get_nbl(); i++){ //ciclo sui blocchi
    ...
    SYS.block_reset(i+1);//resetto gli accumulatori di blocco del blocco 1
  }
