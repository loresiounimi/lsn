...
//matrice che genera l'individuo iniziale secondo le condizioni del problema TSP di città entro
//quadrato di lato 2. In questo esercizio l'individuo iniziale viene caricato da un file e il TSP
//coinvolge i 110 capoluoghi di provincia. Questa funzione dunque non è utilizzata
...
mat Individuo :: genera_individuo(){
    ...
    uvec shuffled = shuffle(regspace<uvec>(1, _cities - 1));//uvec è un vettore di interi. shuffle mescola il vettore di interi creato con indici da 1 a _cities-1. l'indice 0 è fisso essendo la posizione iniziale
    //sostituisco le righe da 1 a _cities-1 (1 è la seconda riga) di nuovo individuo con le righe di individuo in ordine shuffled
...
void Individuo :: mutazione_semplice(){
    ...
    int change1 = static_cast<int>(_rnd->Rannyu(1.0, (double)_cities));
    int change2 = static_cast<int>(_rnd->Rannyu(1.0, (double)_cities));
    if (change1 != change2) {
        _individuo.swap_rows(change1, change2);// swap rows scambio le due righe selezionate da individuo
    }
    this->check();
}
...
//imposta il numero di città
void Individuo :: set_cities(int cities){
  _cities = cities;
  }

  //carica l'individuo iniziale dal file output.dat che contiene i dati organizzati in questo modo:
  //nome città | numero etichetta città | longitudine | latitudine
void Individuo :: carica_individuo_iniziale(){
  ifstream individuo_file("INPUT/output.dat");
  if (!individuo_file.is_open() ) {
        cerr << "Errore nell'apertura dei file." << endl;
    }
    int cities=0;
    string line;
    if (getline(individuo_file, line)) 
    while (getline(individuo_file, line)) {
        if (!line.empty()) { 
            cities++;
        }
    }
    //in base al numero di città presenti nel file, imposto il numero di città di individuo e la sua dimensione
    this->set_cities(cities);
    _individuo.set_size(_cities,3);
    //preparo seconda lettura, nella prima ho solo contato il numero di città per preparare
    //la matrice individuo correttamente
    individuo_file.clear();
    individuo_file.seekg(0, ios::beg);
    //leggo la prima riga di intestazione e la ignoro
    getline(individuo_file, line);
    int i = 0;
    double val1,val2, val3;
    // Leggi dati e li carico in individuo
    while (individuo_file >> line >> val1 >> val2 >> val3) {
        _individuo(i, 0) = val1;
        _individuo(i, 1) = val2;
        _individuo(i, 2) = val3;
        i++;
