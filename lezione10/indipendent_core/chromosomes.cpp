...
void Chromosomes :: inizialize_rnd(){
  ...
  if (input.is_open()){
           ...
           for(int i=0;i<4;i++)seed[i] += _rank; //in questo modo il seme dei rnd di ogni processo è diverso-> ho N=rank rnd diversi
           _rnd.SetRandom(seed,p1,p2);
  } else cerr << "PROBLEM: Unable to open seed.out" << endl;
  input.close();
  //_individuo.inizialize_rnd(_rank);
...
//genera la prima popolazione
    void Chromosomes :: inizialize_population(){
        ...
        _individuo.carica_individuo_iniziale();//carico il primo individuo
        this->set_cities(_individuo.cities());//imposto il numero di città
        _popolazione.set_size(_population);//imposto le dimensioni della popolazione
        //inizializzo i file su cui scrivo l'andamento della miglior stima di L2(x) e di <L2(x)> calcolato su metà popolazione
        ostringstream filename;
        filename << "OUTPUT/best_path_" << _rank << ".dat";
        ostringstream filename1;
        filename1 << "OUTPUT/best_path_half_population_" << _rank << ".dat";
        ofstream coutf;
        coutf.open(filename.str());
        ofstream coutf1;
        coutf1.open(filename1.str());
...
/*for(int i=_elitism;i<_population;i++){
  if(_rnd.Rannyu(0.0,1.0)>0.4){ //noto che con maggiore probabilità di mutazione ottengo un risultato migliore di L(x) con un numero minore di generazioni. Ne risente <L1(x)> che ha maggiori fluttuazioni.
 ...
 for(int i=_elitism;i<_population;i++){
  ...
  ostringstream filename1;
  filename1 << "OUTPUT/best_path_half_population_" << _rank << ".dat";
  coutf.open(filename.str(),ios::app);
  coutf << _ngeneration << setw(29) << _popolazione[0].costo() << endl;
  coutf1.open(filename1.str(),ios::app);
  double dep=0.0;
  double r=static_cast<double>(_population)/2.0;
  for(int i=0;i<static_cast<int>(r);i++){
      dep+=_popolazione[i].costo();
...
void Chromosomes :: set_cities(int cities){
  _cities = cities;
  }
//imposta il rank del core
void Chromosomes :: set_rank(int rank){
  _rank = rank;
  }
...
//imposta l'individuo i della popolazione
void Chromosomes :: set_individuo(int i, mat a){
  _popolazione[i].set_individuo(a);
  }