...
double Random :: funzionesimil(){ 
  //questa funzione genera un numero casuale secondo la distribuzione 2(x-1)
  double dep=1-sqrt((1-Rannyu()));
  return dep;
}


void Random :: shuffle(vector <int> &v){
  //questa funzione mescole le componenti di un vector
    for (int i = v.size() - 1; i > 0; --i) {
    int j = static_cast<int>(this->Rannyu(0, i + 1));
    swap(v[i], v[j]);
  }
