...
for i in range(4):
  plt.figure()
  if(i==3):
   counts, bins = np.histogram(std[:,i], bins=100)
  ...
  plt.hist(std[:, i], bins=100, color='skyblue', edgecolor='black')  # Istogramma
  if(i==3):
...
for i in range(4):
 plt.figure()  
 if(i==3):
   counts, bins = np.histogram(cauchy[:,i], bins=100, range=(-25,25))
   bin_centers = (bins[:-1] + bins[1:]) / 2
 ...
 plt.hist(cauchy[:, i], range=(-25,25), bins=100, color='skyblue', edgecolor='black')  
 if(i==3):
    x_fit = np.linspace(bin_centers[0], bin_centers[-1], 1000) 
